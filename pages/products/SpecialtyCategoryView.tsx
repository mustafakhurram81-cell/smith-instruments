...
import { useSpecialtySubcategories, useProductsBySpecialty } from '../../lib/queries';
import { ChevronRight, Package, ArrowRight, Stethoscope } from 'lucide-react';
import { ProductCard } from '../../components/ProductCard';

export const SpecialtyCategoryView: React.FC = () => {
        ...
        return (
            <div className="pt-20 min-h-screen bg-stone-50">
                <SEO
                    ...
                    description={`Browse our collection of ${subcategory.toLowerCase()} surgical instruments for ${category.toLowerCase()}.`}
                />
                ...
                <div className="bg-brand-charcoal text-white py-12 md:py-20 relative overflow-hidden">
                    <div className="container mx-auto px-6 relative z-10">
                        <div className="flex items-center gap-2 text-xs text-stone-400 mb-4 uppercase tracking-widest">
                            ...
                            <Link to={`/products/specialty/${encodeURIComponent(category)}`} className="hover:text-white">{category}</Link>
                            <ChevronRight size={12} />
    ...
    // Show subcategories for this specialty category
    return (
        <div className="pt-20 min-h-screen bg-stone-50">
            <SEO
                title={`${category} - Surgical Instruments`}
                description={`Browse our collection of surgical instruments for ${category.toLowerCase()}.`}
            />
            ...
            <div className="bg-brand-charcoal text-white py-12 md:py-20 relative overflow-hidden">
                <div className="container mx-auto px-6 relative z-10">
                    ...
                    <p className="text-stone-400 font-light max-w-2xl text-lg">
                        {!loading && (
                            <>{totalProducts} instruments across {subcategories.length} procedures</>
                        )}
                    </p>
                </div>
                <div className="absolute top-0 left-0 w-full h-full opacity-10 pointer-events-none">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                                {subcategories.map((sub, idx) => (
                                    <FadeIn key={sub.name} delay={idx * 0.05}>
                                        <div
                                            onClick={() => navigate(`/products/specialty/${encodeURIComponent(category)}/${encodeURIComponent(sub.name)}`)}
