...
export const Dashboard: React.FC = () => {
    ...
    const fetchStats = async () => {
        ...
        const catStats = Object.entries(catCounts).map(([name, count]) => ({ name, count })).sort((a, b) => b.count - a.count);

        setCategoryStats(catStats);
        setStats({
            products: total || 0,
            categories: catStats.length,
        ...
        <div className="min-h-screen bg-stone-100">
            ...
            <div className="max-w-7xl mx-auto p-6">
                ...
                {activeTab === 'categories' && (
                    <div className="bg-white rounded-xl shadow-sm p-6">
                        <h2 className="font-medium text-brand-charcoal mb-4">Manage Categories</h2>
                        <div className="space-y-2">
                            {categoryStats.map(cat => (
                                <div key={cat.name} className="flex items-center justify-between p-3 bg-stone-50 rounded-lg">
                                    <div className="flex items-center gap-3">
                                        <FolderOpen size={18} className="text-brand-gold" />
                                        <span className="font-medium">{cat.name}</span>
                                        <span className="text-sm text-stone-500">({cat.count} products)</span>
                                    </div>
                                    <div className="flex gap-2">
                                        <button
